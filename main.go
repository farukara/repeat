...
func main() {
	...
	if _, ok := Reptimes[args[2]]; !ok {
		...
		fmt.Println("1d 2d 3d 5d 1w 2w 3w 1m 2m 3m 4m 5m 6m 7m 8m 9m 10m 11m 1y 2y 3y 4y 5y")
	...
	if (duedate.Sub(time.Now())) > time.Hour*720 {
		fmt.Println("\033[7;33mWARNING: you are editing a task that is due in more than 1 month")
